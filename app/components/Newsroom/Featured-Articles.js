...
export default function FeaturedArticles(){
    return(
        <div className="border border-primary rounded-[10px] p-5 relative ">
            ...
            <div className="flex gap-4 flex-col h-[460px] overflow-y-scroll scrollbar-hide">
                {Articles.map((article) => (
                    <div key={article.id} className="border border-primary rounded-[10px] p-1 flex gap-2 mb-1">
                        ...
                        <div className="flex flex-col gap-2 pr-1 justify-between">
                            ...
                            <div className="flex gap-2 flex-wrap-reverse md:flex-nowrap items-end justify-between">
                                <p className="md:text-xs text-[10px] flex gap-1 items-center"><MdOutlineCalendarMonth className="text-gray-400 text-md" /> {article.date}</p>
                                <p className="md:text-xs text-[10px] text-primary">{article.status}</p>
                                <Button variant="blue" href="/#" className="text-sm rounded-md px-2 py-1">{article.channel}</Button>
                            </div>
                        </div>
                    </div>
                ))}
